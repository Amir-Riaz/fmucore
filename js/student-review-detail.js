...
guardPage({
  ...
  onReady: async (user, profile) => {
    ...
    renderTopbar("student-review-abstracts", { isAdmin: profile.role === "admin" });
...
async function loadReview() {
  try {
    ...
    // Students may only open abstracts assigned to them; admins can open any (QA).
    const isAdmin = currentProfile.role === "admin";
    if (!isAdmin && currentReview.assignedStudentReviewerUid !== currentUser.uid) {
      document.getElementById("loadingState").classList.add("hidden");
      document.getElementById("forbiddenState").classList.remove("hidden");
      return;
    }
...
async function saveRecommendation() {
  ...
  const updates = {
    studentReviewDecision: decision,
    studentReviewTrack: track,
    studentReviewedByUid: decision ? (currentUser?.uid || null) : null,
    studentReviewedByName: decision ? (currentProfile?.fullName || currentUser?.email || null) : null,
    studentReviewedAt: decision ? serverTimestamp() : null,
  };
  ...
  try {
    // Student reviewers only ever write to the anonymized review view —
